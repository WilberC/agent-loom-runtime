...
The runtime also advertises `repo.sync` for repository automation applications.
It supports `pull`, `push`, and `pull_push` operations: repository URLs must use
HTTPS, worktrees are relative to `AGENT_LOOM_REPO_SYNC_ROOT` (default `repos`),
and push operations require an existing worktree. Credentials are provided
ephemerally by the control plane. The credential is passed to Git through
environment-backed HTTP configuration and is never stored in the local command
payload. The initial `dotfiles-skills` configuration remains pull-only until a
push-enabled configuration is explicitly authorized.
